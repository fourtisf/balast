...
describe('fees earned per day, as this browser measured them', () => {
  it('is each day’s growth since the previous day recorded, valued at today’s prices', () => {
    ...
    // Day one carries everything since the mint (10 + 1×2) and says so; day two: 5 + 2×2.
    expect(out.values).toEqual([12, 9, 0]);
    expect(out.firstIncludesEarlier).toBe(true);
  });
  ...
  it('counts the first day from zero when the position was minted that day', () => {
    ...
    const out = dailyEarnedUsd('0xab', [r(4n, 0n, '2026-09-20T01:00:00Z')], { now: T0, store });
    expect(out.values).toEqual([4]);
    expect(out.firstIncludesEarlier).toBe(false);
  });
  ...
  it('never counts a fall as negative fees', () => {
    ...
    expect(dailyEarnedUsd('0xab', [r(3n, 0n)], { now: T0 + DAY, store }).values).toEqual([10, 0]);
  });

  it('draws nothing for a wallet it has never measured', () => {
    expect(dailyEarnedUsd('0xcd', [r(1n, 1n)], { now: T0, store: {} })).toEqual({ values: [], since: null, firstIncludesEarlier: false });
  });
});
