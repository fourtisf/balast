 ...
 * it right without a re-sync — and the tables afterwards have to equal a
 * clean sync's, as text (§9).
 */
...
describe('rows and a cursor stamped before the floor', () => {
  ...
  it('finds nothing to do on a healthy database', async () => {
    ...
    const done = await repair.repairBlockTimes();
    expect(done).toEqual({ rows: 0, hours: 0, rewoundTo: null, cursorTimeFixed: false });
  ...
  it('rewinds past the bad rows on the first pass, and the tables end up as a clean sync leaves them', async () => {
    ...
    // A new process: the repair runs ahead of its first pass, the pass
    // re-reads the rewound range, and the priced tables are rebuilt in full.
    const source = new FixtureLogSource(chain);
    const p = poller(source);
    const pass = await p.runPass();
    expect(pass.fromBlock).toBeLessThanOrEqual(bad);
    await p.syncToHead();
  ...
  it('restores the cursor time from the newest row when only the cursor was wrong', async () => {
    ...
    expect(done.rewoundTo).toBeNull();
